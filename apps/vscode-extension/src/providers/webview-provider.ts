...
export class WebviewProvider {
  private readonly _viewType = 'sector.webview';
  private _panel: vscode.WebviewPanel | undefined = undefined;

  constructor(
    private readonly _extensionUri: vscode.Uri,
    private readonly _sectorClient: SectorClient
  ) {}

  showAssistant(): void {
    this._showWebview('Sector AI Assistant', 'assistant');
  }
  ...
  private async _handleSendPrompt(data: any): Promise<void> {
    try {
      const response = await this._sectorClient.sendPrompt(data);
  ...
  private async _handleCreateVisualization(data: any): Promise<void> {
    try {
      const response = await this._sectorClient.createVisualization(data);
      this._panel?.webview.postMessage({ type: 'response', data: response });
    } catch (error) {
      this._panel?.webview.postMessage({ 
        type: 'error', 
