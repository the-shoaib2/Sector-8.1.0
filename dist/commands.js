...
function registerCommands(context) {
    const disposables = [];
    // Login command
    disposables.push(vscode.commands.registerCommand('synapse.login', async () => {
    ...
    // Logout command
    disposables.push(vscode.commands.registerCommand('synapse.logout', () => {
        // TODO: Implement actual logout
        vscode.window.showInformationMessage('Logout functionality coming soon!');
    }));
    // Open project command
    disposables.push(vscode.commands.registerCommand('synapse.openProject', async () => {
    ...
    disposables.push(vscode.commands.registerCommand('synapse.showAssistant', () => {
    ...
    disposables.push(vscode.commands.registerCommand('synapse.visualizeCode', () => {
    ...
    disposables.push(vscode.commands.registerCommand('synapse.analyzeContext', () => {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            // This will be handled by the WebviewProvider
    ...
    disposables.push(vscode.commands.registerCommand('synapse.exportVisualization', async () => {
    ...
    disposables.push(vscode.commands.registerCommand('synapse.createProject', async () => {
        const projectName = await vscode.window.showInputBox({
            prompt: 'Enter project name',
    ...
    disposables.push(vscode.commands.registerCommand('synapse.executeCode', async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showWarningMessage('No active editor found');
            return;
