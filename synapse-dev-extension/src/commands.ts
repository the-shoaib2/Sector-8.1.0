...
export function registerCommands(context: vscode.ExtensionContext): vscode.Disposable[] {
  ...
  // Login command
  disposables.push(
    vscode.commands.registerCommand('sector.login', async () => {
  ...
  disposables.push(
    vscode.commands.registerCommand('sector.openProject', async () => {
  ...
  disposables.push(
    vscode.commands.registerCommand('sector.exportVisualization', async () => {
  ...
  disposables.push(
    vscode.commands.registerCommand('sector.createProject', async () => {
      const projectName = await vscode.window.showInputBox({
        prompt: 'Enter project name',
  ...
  disposables.push(
    vscode.commands.registerCommand('sector.executeCode', async () => {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage('No active editor found');
        return;
